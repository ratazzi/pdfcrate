...
//! - Page 6: Interactive forms (AcroForms)
//! - Page 7: PDF embedding and merging
//!
...
fn add_page_forms(doc: &mut Document) -> PdfResult<()> {
    ...
    doc.font("Helvetica").size(11.0);
    doc.text_at(
        "Page 6: AcroForms - text fields, checkboxes, and dropdowns",
        [margin, 780.0],
    );
...
fn add_page_pdf_embed(doc: &mut Document) -> PdfResult<()> {
    ...
    doc.font("Helvetica").size(11.0);
    doc.text_at(
        "Page 7: Embed and draw pages from other PDFs",
        [margin, 780.0],
    );
